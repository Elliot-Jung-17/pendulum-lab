...
- **3D Lab (Govern → 3D Lab)** — a **rope/string pendulum** (hybrid taut/slack dynamics with tension readout, slack warnings, inelastic capture) and a **true spherical pendulum** (θ̈ = sinθcosθφ̇² − (g/l)sinθ with conserved E and L_z diagnostics) rendered through an orbit camera (drag to rotate, wheel to zoom) with a θ̇=0 Poincaré inset and exportable 3D diagnostic snapshots.
