...
test('analysis superpack: Wada convergence, Melnikov, fixed point, shadowing', async ({ page }) => {
  ...
  await expect(page.locator('#rwSuperpackResults')).toContainText(/Poincaré Fixed Point — (STABLE|UNSTABLE|SADDLE|CENTER|STABLE-NODE|STABLE-SPIRAL|UNSTABLE-NODE|UNSTABLE-SPIRAL|PERIOD-DOUBLING-CRITICAL|FOLD-CRITICAL|DEGENERATE)/i, { timeout: 30_000 });
  ...
  await expect(page.locator('#rwSuperpackResults')).toContainText(/Shadowing Reliability — score \d+%/, { timeout: 60_000 });

  // Wada resolution convergence — the headline multi-resolution analysis.
  await page.locator('#rwSpWada').click();
  await expect(page.locator('#rwSuperpackResults')).toContainText(/Wada Resolution Convergence — (STABLE-WADA-EVIDENCE|STABLE-NON-WADA|UNSTABLE|INSUFFICIENT-DATA)/, { timeout: 120_000 });
