...
export async function analyzeChainDiagnostics(): Promise<void> {
  ...
  setText('d3Analysis', `Computing λ/RQA/FTLE for the N=${spec.masses.length} spherical chain ${chainClient().usesWorker() ? '(worker)' : '(main thread)'}…`);
  try {
...
export async function analyzeChainSpectrum(): Promise<void> {
  ...
  setText('d3Analysis', `Computing the full ${dim}-exponent Lyapunov spectrum (N=${spec.masses.length} chain, dt=0.002) ${chainClient().usesWorker() ? '(worker)' : '(main thread)'}…`);
  try {
    const result = await chainClient().lyapunovSpectrum(spec, state0, dim, { dt: 0.002, steps: 6000 });
    const lines = result.spectrum.map((lambda, i) =>
      `λ${i + 1}=${lambda.toFixed(4)}±${(result.blockStdError[i] ?? 0).toFixed(4)}`);
    const consistency = result.consistency;
    const consistencyLine = spec.damping === 0
...
export function runChainEnergyShell(): void {
  ...
  window.setTimeout(() => {
    try {
      ...
      attachBadge('d3ShellInfo', conservative ? (tight ? 'validated' : 'finite-time-estimate') : 'caveat', conservative
        ? (tight
...
function drawShellTraces(times: number[], eDev: number[], lDev: number[], horizon: number): void {
  ...
  const maxAbs = Math.max(
    1e-16,
    ...eDev.map((value) => Math.abs(value)),
    ...lDev.map((value) => Math.abs(value))
  );
