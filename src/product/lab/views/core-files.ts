...
import { toCanonicalPlanar, type PlanarConfig } from '../../adapters/physics/planar';
import { createExperimentRoute } from '../../persistence';
import { exportFigure } from './core-plots';
import {
...
const trays = new WeakMap<CoreModel, PlanarConfig[]>();

export function createCoreFiles(
  document: Document,
  model: CoreModel,
  restore: (config: PlanarConfig) => void,
  canUse: () => boolean
) {
  ...
  const save = createButton(document, {
    ...
    onClick() {
      try {
        saveCoreConfig(document.defaultView!.localStorage, model.state.config);
  ...
  const exports = createSection(document, {
    ...
    title: '내보내기와 복원',
    description:
      '상태 JSON은 SI 초기조건·적분기·분석 설정입니다. CSV는 기록된 표본, SVG는 상태/시간 그래프입니다. 저장된 결과에서 계산을 이어가는 파일은 아닙니다.'
  });
  ...
  const json = createButton(document, {
    label: '상태 JSON 다운로드',
    onClick() {
      download(serializeCoreConfig(model.state.config), 'application/json', 'json');
    }
  });
  const csv = createButton(document, {
    ...
    variant: 'secondary',
    onClick() {
      download(trajectoryCsv(model.state.samples), 'text/csv;charset=utf-8', 'csv');
    }
  });
  const figure = createButton(document, {
    ...
    variant: 'secondary',
    onClick() {
      download(exportFigure(document, model.state.config, model.state.samples), 'image/svg+xml', 'svg');
    }
  });
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      if (file.size > PLANAR_IMPORT_MAX_BYTES) throw new Error('상태 파일은 200 KB 이하여야 합니다.');
      const text = await file.text();
      if (disposed || generation !== importGeneration) return;
      if (!canUse()) throw new Error('실행과 분석을 멈춘 뒤 다시 가져오세요.');
      const config = parseCoreConfig(text, model.state.config.systemId);
      restore(config);
  ...
  const makeShare = createButton(document, {
    ...
    variant: 'secondary',
    onClick() {
      const state = toCanonicalPlanar(model.state.config);
  ...
  exports.body.append(
    ...
    element(
      ...
      'p',
      'lab-muted',
      '그림은 선당 최대 약 600점으로 축약합니다. CSV 표본 간격은 시간 간격 × sampleEvery이며 마지막 상태를 포함합니다. 새 저장 키만 사용하고 기존 앱 자료는 변경하지 않습니다.'
    )
  );
  function update() {
