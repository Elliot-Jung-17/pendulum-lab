...
function codeCell(lines: string[]): NotebookCell {
  return { cell_type: 'code', metadata: {}, execution_count: null, outputs: [], source: lines.map((line) => `${line}\n`) };
}
...
export function buildNotebookV2(input: NotebookInput): NotebookDocument {
  ...
  if (input.studyCsv !== null) {
    ...
    cells.push(markdownCell(['_No parameter study was generated at export time, so the λ(parameter) plot cell is omitted._']));
  }
...
export function validateNotebook(value: unknown): NotebookValidation {
  ...
  if (typeof value !== 'object' || value === null) return { ok: false, problems: ['notebook is not an object'], cellCount: 0, codeCells: 0 };
  ...
  nb.cells.forEach((cell, index) => {
    if (cell.cell_type !== 'markdown' && cell.cell_type !== 'code') problems.push(`cell ${index} has invalid type ${String(cell.cell_type)}`);
    if (!Array.isArray(cell.source) || cell.source.some((line) => typeof line !== 'string')) problems.push(`cell ${index} source must be a string array`);
