...
describe('progressive renderer', () => {
  it('draws every item exactly once across chunks', async () => {
    ...
    const result = await renderProgressively(1000, (start, end) => {
  ...
  it('stops between chunks when cancelled (backpressure)', async () => {
    let calls = 0;
    const result = await renderProgressively(10_000, () => {
      calls += 1;
  ...
  it('adapts the chunk size downward when a chunk overruns the budget', async () => {
    let clock = 0;
    const result = await renderProgressively(400, () => {
...
describe('performance budget evaluation', () => {
  ...
  it('flags each violated budget independently', () => {
    expect(evaluatePerformanceBudget({ ...healthy, fps: 15 }).find((row) => row.metric === 'frame rate')!.ok).toBe(false);
    expect(evaluatePerformanceBudget({ ...healthy, physicsMsPerFrame: 20 }).find((row) => row.metric === 'physics per frame')!.ok).toBe(false);
    expect(evaluatePerformanceBudget({ ...healthy, usedHeapBytes: 1800 * 1024 * 1024 }).find((row) => row.metric === 'js heap')!.ok).toBe(false);
    expect(evaluatePerformanceBudget({ ...healthy, jobsInFlight: 99 }).find((row) => row.metric === 'worker jobs in flight')!.ok).toBe(false);
    expect(evaluatePerformanceBudget({ ...healthy, localStorageBytes: 5 * 1024 * 1024 }).find((row) => row.metric === 'localStorage payload')!.ok).toBe(false);
    expect(evaluatePerformanceBudget({ ...healthy, idbUsageFraction: 0.95 }).find((row) => row.metric === 'IndexedDB quota')!.ok).toBe(false);
  });
